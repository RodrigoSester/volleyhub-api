...
export function up(knex) {
  return knex.schema.raw(`
    ...
    DROP INDEX IF EXISTS uq_teams_modality_created_by;

    CREATE UNIQUE INDEX idx_team_modality_created_by ON teams (modality, created_by) WHERE NOT is_deleted;
    ALTER TABLE teams ADD CONSTRAINT uq_teams_modality_created_by UNIQUE (modality, created_by, is_deleted);
  `);
};
...
export function down(knex) {
  return knex.schema.raw(`
    ALTER TABLE teams DROP CONSTRAINT IF EXISTS uq_teams_modality_created_by;
    DROP INDEX IF EXISTS idx_team_modality_created_by;
